...
2. 在"环境配置切换"区域：
   - **当前正在用的配置**会高亮（浅绿底 + 左侧绿条）并显示绿色「使用中」胶囊——判定依据是 `.env` 与该 `.env.xxx` 内容 md5 完全一致（切换是逐字节拷贝，注释 / 引号 / 末尾空行都不影响），所以"真正套用中"哪一份一目了然。
...
- WSL 路径（`\\wsl.localhost\...`）：chokidar 不可靠，已改为**定时轮询兜底**（每 3 秒列举并重算 md5），变化一般会在数秒内自动刷新；若仍想立即刷新，点头部「刷新数据」按钮即可手动拉取全部项目。
